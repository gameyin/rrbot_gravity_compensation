...
class Kinematics {
 public:
  ...
  bool getGravityTorques(const sensor_msgs::JointState joint_configuration, baxter_core_msgs::SEAJointState &left_gravity, bool isEnabled);

 private:
  ros::NodeHandle nh, nh_private;
  std::string root_name, tip_name, grav_arm_name;
  KDL::JntArray joint_min, joint_max;
  KDL::Chain chain, grav_chain_arm;
  unsigned int num_joints;

  KDL::ChainFkSolverPos_recursive* fk_solver;
  KDL::ChainIkSolverPos_NR_JL *ik_solver_pos;
  KDL::ChainIkSolverVel_pinv* ik_solver_vel;
  KDL::ChainIdSolver_RNE *gravity_solver_arm;
  ...
  std::vector<int> indd;
  std::vector<std::string> arm_joint;
